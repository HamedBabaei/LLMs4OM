...
def postprocess_hybrid(
    predicts: List, ir_score_threshold: float = 0.9, llm_confidence_th: float = 0.7
) -> [List, Dict]:
    ir_outputs = predicts[0]["ir-outputs"]
    llm_outputs = predicts[1]["llm-output"]
    ...
    targets = [
        target
        for index, ir_output in enumerate(ir_outputs)
        for target in ir_output["target-cands"]
    ]
    targets = list(set(targets))
    target2index = {target: index for index, target in enumerate(targets)}
    source2index = {
        ir_output["source"]: index for index, ir_output in enumerate(ir_outputs)
    }

    ir_dict = {ir_output["source"]: ir_output for ir_output in ir_outputs}
    ir_dict_based_llm = {
        ir_output["source"]: np.zeros(len(target2index)) for ir_output in ir_outputs
    }
    llm_dict = {
        ir_output["source"]: np.zeros(len(target2index)) for ir_output in ir_outputs
    }
    ...
    for llm_output in tqdm(llm_outputs):
        ...
        if llm_output["score"] > llm_confidence_th:
            ...
            for index, (ir_cand, ir_cand_score) in enumerate(
                zip(ir_output["target-cands"], ir_output["score-cands"])
            ):
                if ir_cand == llm_output["target"]:
                    confidence_ratio = (
                        llm_output["score"] * 0.2 + 0.8 * ir_cand_score
                    ) / 2
                    predicts_list = [
                        llm_output["target"],
                        ir_cand_score,
                    ...
                    outputdict[llm_output["source"]].append(predicts_list)
                    ir_dict_based_llm[llm_output["source"]][
                        target2index[ir_cand]
                    ] = ir_cand_score
                    llm_dict[llm_output["source"]][
                        target2index[ir_cand]
                    ] = confidence_ratio
                    # llm_output['score']
                    break
    ...
    for col_idx in range(ir_matrix_based_llm.shape[1]):
        ...
        ir_matrix_based_llm[:, col_idx] = np.where(
            np.arange(len(col)) != max_index, 0, col
        )
    ...
    for row_idx in range(ir_matrix_based_llm.shape[0]):
        ...
        ir_matrix_based_llm[row_idx, :] = np.where(
            np.arange(len(row)) != max_index, 0, row
        )
